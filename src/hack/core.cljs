...
;; Hacking logic
;;
;; tl;dr: build regexps to find solutions.
;;
;; There are two data structures we're working with:
;;
;;   - words, which is a list of strings, and
;;   - results, which is a map storing our hacking
;;     progress so far.
;;
;; For example, below we have three words and we know that the
;; last one's similarity to the answer is 2:
;;
;;   (def words ["foo" "bar" "baz"])
;;   (def results {"baz" 2})
;;
;; Each pair in the results map defines a constraint in the space
;; of solutions to to puzzle. Each constraint is a disjunction
;; which can be expressed with a regular expression.
;;
;; For example, ["baz" 2] can be translated to a disjunction of
...
;;   ([^b]az|b[^a]z|ba[^z])
;;
;; results->predicate takes a results map and translates each pair
;; in the map to a regular expression. It returns a function which
;; returns true iff a string passed as an argument matches all
;; regular expressions.
;;
;; To generate all possible solutions filter the list of words
;; using the predicate generated by results->predicate. It tends
;; to work.
